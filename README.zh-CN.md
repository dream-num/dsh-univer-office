...
支持的 DSH 版本为 `0.1.5-rc.3`、`0.1.7-alpha.2` 和 `0.1.7-rc.1`。
...
- 打开旧版 `.univer` 文件时会自动升级到 v3 格式：先在原文件旁保存备份，在临时副本中完成迁移和验证，成功后才替换原文件；升级失败时原文件保持不变。再次打开同一份未变化的文件时复用这份备份，不会再复制一份。文件优化仅升级输出副本，不修改源文件。
