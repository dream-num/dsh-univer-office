...
- **文件树按需预览**：在 DSH 文件树中点击 `.univer` 文件即可在右侧栏打开预览，与 `.docx`、`.xlsx`、`.pptx` 的内置预览一致。整个过程不需要 Agent 执行任何操作，因此在关闭自动实时预览后依然可用；文件存在草稿或已提交 worktree 时会显示切换项，打开文件不会隐藏进行中的修改。该入口直接注册在 DSH 自带的右侧栏上；若安装了 `dsh-better-sidebar`，由于该插件会接管文件地址，预览会改为注册成它的文件预览器。
...
- **会话审阅卡片**：每个修改过的 `.univer` 文件都有独立的完整预览卡片；已删除的临时文件不会留下无效卡片。可在插件设置中关闭「在会话中显示审阅卡片」让会话更紧凑——文件树预览与实时窗口由各自的开关独立控制。
  ...
  两个开关同时兼容旧版 DSH 设置与 0.1.7-alpha.1 起的 profile 配置表单。新版中配置区位于「包含的组件」上方，点击「保存」后实时生效。旧版使用独立的偏好存储，升级后可能需要重新设置。
