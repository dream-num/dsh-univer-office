...
Gateway 还为 trunk revision 组合 Collaboration SDK History Service 与 Endpoint。Gateway 自己在共享 `.univer` connection 上实现 History database adapter。`history@2` 不再按 revision 派生：每个 Unit 只有一条**分段起始**记录（主键 `(unit_id, start_revision)`），`endRevision` 由下一段的起始 revision 计算、不入库，列表按 `startRevision` 降序并以排他的 `beforeRevision` 分页，`hasMore` 只表示页下方仍存在匹配记录。SDK 1.0.0 移除了旧的逐 revision History 读取与重建 API，因此文件 runtime 启动时不再 reconcile 到 trunk head，SDK transport 直接开放。Viewer 为 trunk 的 Sheet、Doc、Slide、Base 与 Board 分别注册官方 History UI；只读审阅可检查历史但不能恢复，可编辑 trunk 视图可显式恢复，worktree 与 merge preview 不呈现历史入口。

`.univer` 内的 `collaboration_schema_versions` 逐 component 记录 schema 版本（core、worktree、assets、history），它与文件格式版本 v0/v1/v2 是两条独立的轴：后者描述容器布局，前者描述 Gateway adapter 拥有的表结构。要改变 cohort 版本，先读该版本的 Collaboration SDK migration notes（见 `AGENTS.md`），它是这些 component 版本的权威来源。打开早于当前 component 版本的文件时，`src/gateway-app/univerfile-sqlite/migration/` 先写备份，再按 core → worktree → history 就地迁移，每个 component 提交自己的事务以便中断后可继续；无法安全迁移的文件（例如某个 Unit 的 History revision 缺少分段起始记录）会带 Unit 名明确拒绝打开并保留原状，不会被静默当作当前版本服务。Unit 与 Worktree Unit 的创建身份取自 SDK 的 `UnitRecord.creatorID`/`createdAt`：trunk 接入的 Worktree Unit 继承 trunk Unit 的创建信息，迁移无法还原时回落到 `anonymous` 与文件自身的创建时间。
